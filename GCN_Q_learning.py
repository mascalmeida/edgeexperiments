...
for i, edge_server_id in enumerate(edge_server_ids):
    ...
    #edge_server_data.to_csv('GCN_Q_learning_migration'+'_id'+ str(edge_server_id) +'_'+'.csv', index=False)
...
df.to_csv('GCN_Q_learning_migration.csv', index=False)
...
df_power.to_csv('GCN_Q_learning_migration_power_list.csv', index=False)
