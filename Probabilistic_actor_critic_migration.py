...
for i, edge_server_id in enumerate(edge_server_ids):
    ...
    #edge_server_data.to_csv('Probabilistic_actor_critic_migration'+'_id'+ str(edge_server_id) +'_'+'.csv', index=False)
...
df.to_csv('Probabilistic_actor_critic_migration.csv', index=False)
...
df_power.to_csv('Probabilistic_actor_critic_migration_power_list.csv', index=False)
